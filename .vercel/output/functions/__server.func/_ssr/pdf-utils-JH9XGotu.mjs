...
//#region node_modules/.nitro/vite/services/ssr/assets/pdf-utils-JH9XGotu.js
...
function generateDetailedOrdersPDF(orders, brand = defaultBrand) {
	const doc = new import_jspdf_node_min.default({
		unit: "mm",
		format: "a4",
	...
	orders.forEach((order, idx) => {
		...
		let cy = y + 11;
		if (email) {
			doc.text(email, 14, cy);
			cy += 4.5;
		}
		if (phone) {
			doc.text(`Phone: ${phone}`, 14, cy);
			cy += 4.5;
		}
		...
		[
			shipAddr.street,
			`${shipAddr.city}${shipAddr.state ? ", " + shipAddr.state : ""} ${shipAddr.zip}`,
			shipAddr.country
		].filter(Boolean).forEach((line) => {
			doc.text(line, 14 + halfW + 8, shipY);
			shipY += 4.5;
		});
		...
		if (order.discount) summaryRows.push({
		...
		summaryRows.forEach((row) => {
			...
			doc.setTextColor(...C.GRAY);
			doc.text(row.label, sumX, ry);
			if (row.label.toLowerCase().includes("discount")) doc.setTextColor(22, 163, 74);
...
//#endregion
export { generateDetailedOrdersPDF as n, generateReceiptPDF as r, generateCustomerProfilePDF as t };
