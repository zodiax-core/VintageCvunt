...
function Auth() {
  ...
  const validate = () => {
    ...
    else if (mode !== "reset" && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email))
    ...
    try {
      if (mode === "register") {
        ...
        if (user.isEmailVerified) {
          ...
          const isAdminRole = isAdminEmail(user.email) || user.role === "admin";
          setSuccessMsg(
            isAdminRole
              ? "Admin account created! Redirecting to Dashboard…"
              : "Account created! Welcome to the house.",
          );
        ...
        const isAdminRole =
          user.email.toLowerCase() === "zodiaxcore@gmail.com" || user.role === "admin";
        setSuccessMsg(
          isAdminRole
  ...
  const getHeaderSubtitle = () => {
    switch (mode) {
      case "login":
        return "Returning Patron";
      case "register":
        return "New Patron";
      case "verify":
        return "Verify Patron";
      case "forgot":
        ...
        <div className="relative mx-auto w-full max-w-md px-6">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {mode === "register" && (
                <div>
                  <label className="block font-mono text-[10px] uppercase tracking-[0.28em] text-chrome-dim mb-2">
                    Full Name
                  </label>
                  <input
                    value={form.name}
                  ...
                  {touched.name && errors.name && (
                    <p className="mt-1 font-mono text-[10px] text-red-400">{errors.name}</p>
                  )}
                </div>
              )}
              ...
              {(mode === "login" || mode === "register" || mode === "forgot") && (
                <div>
                  ...
                  {touched.email && errors.email && (
                    <p className="mt-1 font-mono text-[10px] text-red-400">{errors.email}</p>
                  )}
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute right-3.5 top-1/2 -translate-y-1/2 text-chrome-dim/50 hover:text-chrome-dim transition-colors"
                      tabIndex={-1}
                    >
                  ...
                  {touched.password && errors.password && (
                    <p className="mt-1 font-mono text-[10px] text-red-400">{errors.password}</p>
                  )}
                </div>
              )}
              ...
              {(mode === "verify" || mode === "reset") && (
                <div>
                  ...
                  {touched.otp && errors.otp && (
                    <p className="mt-1 font-mono text-[10px] text-red-400 text-center">
                      {errors.otp}
                    </p>
                  )}
                ...
                {successMsg && (
                  ...
                  >
                    <div className="rounded-xl border border-green-500/30 bg-green-500/10 px-4 py-3 text-center mt-4">
                      <p className="font-mono text-[11px] text-green-400">{successMsg}</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
                    ...
                    <div className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 mt-4">
                      <ul className="space-y-1">
                        {Object.entries(errors)
                          .filter(([, v]) => v)
                          .map(([k, v]) => (
                            <li key={k} className="font-mono text-[10px] text-red-400">
                              {v}
                            </li>
                          ))}
                      </ul>
            ...
            <>
              <div className="relative my-8">
                ...
                <div className="relative flex justify-center">
                  <span className="bg-background px-4 font-mono text-[10px] uppercase tracking-[0.3em] text-chrome-dim">
                    or continue with
                  </span>
                </div>
              </div>
